...
export default function Visualizer({
  ...
  const p5InstanceRef = useRef(null); // Ref to store p5.js instance
  ...
  useEffect(() => {
    ...
    const sketch = (p) => {
      let canvas;
      let brightnessSlider, lengthPowerSlider, minFreqSlider, maxFreqSlider;
      let brightness = brightnessPower;
      let lengthPower = 1; // Initialize lengthPower
      let minFreq, maxFreq;
      ...
      p.setup = () => {
        ...
        brightnessSlider.position(canvasWidth - 150, 1510);
        brightnessSlider.style('width', '140px');
        brightnessSlider.input(() => {
          brightness = brightnessSlider.value();
        });

        // Create the length power slider
        lengthPowerSlider = p.createSlider(0.1, 10, lengthPower, 0.01);
        lengthPowerSlider.position(brightnessSlider.x, 1540);
        lengthPowerSlider.style('width', '140px');
        lengthPowerSlider.input(() => {
          lengthPower = lengthPowerSlider.value();
        });

        // Create the minimum frequency slider (in semitones)
        minFreqSlider = p.createSlider(0, maxSemitone, 24, 0.5);
        minFreqSlider.position(10, 1400);
        ...
        maxFreqSlider = p.createSlider(0, maxSemitone, maxSemitone - 30, 0.5);
        maxFreqSlider.position(canvasWidth / 2 + 10, 1400);
      ...
      p.draw = () => {
        ...
        for (let i = 0; i < dataArray.length; i++) {
            ...
            const normalizedEnergy = p.map(Math.pow(energy, lengthPower), 0, Math.pow(255, lengthPower), 0, middle);
        ...
        p.text(`Brightness: ${brightness.toFixed(2)}`, brightnessSlider.x, 960);
        p.text(`Length Power: ${lengthPower.toFixed(2)}`, lengthPowerSlider.x, 990);
      };
    };
