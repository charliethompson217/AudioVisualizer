...
const ColorSettingsModal = ({ 
  ...
  const handleClose = () => {
    ...
    Cookies.set(COOKIE_NAME, JSON.stringify(localHues), { expires: COOKIE_EXPIRY });
  ...
  return (
    <div className="color-settings-modal">
      <div className="color-settings-content">
        ...
        <div className="color-sliders" style={{ 
          display: 'flex', 
          ...
          {noteNames.map((note, index) => (
            <div key={index} className="color-slider-row" style={{ 
              display: 'flex',
              alignItems: 'center',
              width: '100%'
            }}>
        ...
        <div className="modal-buttons">
          ...
          <button className="close-button" onClick={handleClose}>Save & Close</button>
        </div>
      </div>
    </div>
  );
